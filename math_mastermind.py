...
def generate_response(prompt_str, temperature: float = 0.1) -> str:
...
- Example: "Find the derivative of $\sin(x^3) + \ln(x)$"
""")
        st.markdown("""
**Algebra**: Solve equations, factor polynomials, simplify expressions
- Example: "Solve $2x^2 + 5x - 3 = 0$"
...
""")
    ...
    with col_clear: # Assumed placement for Clear button based on column definition ratio
        if st.button("❓❓❓ Clear Conversation"):
            st.session_state.history = []
            st.rerun()
    ...
    with st.form(key="math_form", clear_on_submit=True):
        user_input = st.text_area(
            "✍️✍️✍️ Enter your math problem here.",
            height=100,
            placeholder="Example: Solve x^2 + 5x + 6 = 0 or Find the integral of 2x + 3",
...
.history-box {
    ...
    padding: 15px;
    background: linear-gradient(135deg, #f5f7fa 0%, #c3cefe 100%);
    border-radius: 10px;
...
""" # End of the st.markdown block for the CSS
        , unsafe_allow_html=True) 

    # Building the HTML for the conversation history
    ...
    for idx, qa in enumerate(st.session_state.history):
        ...
        q_number = total_questions - idx
        
        # Build the HTML content for each QA pair
        history_html += f"""
        <p class="question">Q{q_number}: {qa['question']}</p>
        <span class="difficulty">{qa['difficulty']}</span>
    ...
    # Display the final generated HTML
